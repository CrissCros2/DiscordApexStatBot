...
async def YOURMOM(ctx):
    try:
        author = ctx.message.author
        with open('YourMom.png','rb') as f:
            picture = discord.File(f)
            await ctx.message.channel.send(file=picture)
        dontplay = False
        channel = author.voice.channel
        for x in range(0,len(channel.members)):
            if str(channel.members[x]) == "Emerana#8241":
                dontplay = True
        if dontplay == False:
            vc = await channel.connect()
            vc.play(discord.FFmpegPCMAudio(executable="C:\\Users\\chris\\OneDrive\\Desktop\\ApexStatBot\\ffmpeg.exe", source="YOURMOTHER.mp3"))
        while vc.is_playing():
            await asyncio.sleep(1)
        vc.stop()
        server = ctx.message.guild.voice_client
...
@bot.command()                                                                   
async def hoiyaa(ctx):
    try:
        author = ctx.message.author
        channel = author.voice.channel
        vc = await channel.connect()
        vc.play(discord.FFmpegPCMAudio(executable="C:\\Users\\chris\\OneDrive\\Desktop\\ApexStatBot\\ffmpeg.exe", source="HeeeeYuu.mp3"))
        while vc.is_playing():
            await asyncio.sleep(1)
        vc.stop()
        server = ctx.message.guild.voice_client
...
bot.run('ODI4ODg3MTgxOTMzODA1NjEw.YGwHWA.oqoAY7tJGfm1NEhqTpp5z9OI0Zw')
